...
- Updated current installation and adoption examples to `v4.8.49`.
...
### Added
- Added a Syft-generated `container-os-sbom.cdx.json` to the GHCR release workflow so release
  digests have both a SkillOps package SBOM and an OS/image filesystem SBOM.
- Added an `--check-ghcr-os-sbom` post-release smoke option that verifies the release digest has a
  CycloneDX attestation generated by Syft/Anchore.

### Changed
- The GitHub Actions `Post-release Smoke` workflow now verifies the OS/image SBOM attestation when
  `full_smoke` is enabled.
- Updated supply-chain, Docker, release verification, production adoption, and roadmap docs to
  describe package SBOM versus OS/image SBOM boundaries.
