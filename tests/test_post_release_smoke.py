...
def test_default_timeout_gives_ghcr_attestation_room():
...
def test_slsa_readiness_report_check_generates_schema_valid_report():
    ...
    assert checks == [Check("slsa-readiness-report", True, "schema valid; formal_claim=false")]
...
def test_collect_checks_flags_slsa_readiness_without_ghcr_digest():
    args = smoke.argparse.Namespace(
        ...
        skip_pypi=True,
        skip_ghcr=True,
        check_pypi_install=False,
        check_pypi_hash_lock=False,
        check_ghcr_signature=False,
        check_ghcr_os_sbom=False,
        check_slsa_readiness=True,
        check_new_user_path=False,
        python="python3.12",
        require_platform=["linux/amd64", "linux/arm64"],
        no_ghcr_attestations=False,
    )
...
def test_wheel_requirement_line_generates_pip_hash(tmp_path):
    wheel = tmp_path / "skills_orchestrator-4.8.34-py3-none-any.whl"
    wheel.write_bytes(b"fake wheel bytes")

    line = wheel_requirement_line(wheel)

    assert line.startswith("skills-orchestrator==4.8.34 --hash=sha256:")
...
def test_pypi_install_smoke_checks_stable_schema_audit_for_new_user_path(monkeypatch):
    ...
    def fake_run_command(command, *, cwd=None, timeout=120):
        ...
        if command[-1] == "--version":
            return smoke.subprocess.CompletedProcess(
                command, 0, "skills-orchestrator, version 4.8.34\n", ""
            )
        if command[1:4] == ["mcp-test", "list_skills", "{}"]:
            return smoke.subprocess.CompletedProcess(
                command,
...
def test_pypi_hash_locked_install_smoke_builds_local_wheelhouse(monkeypatch):
    ...
    def fake_run_command(command, *, cwd=None, timeout=120):
        ...
        if command[1:4] == ["-m", "pip", "download"]:
            ...
            (smoke.Path(wheelhouse) / "skills_orchestrator-4.8.34-py3-none-any.whl").write_bytes(
                b"skillops"
            )
        ...
        if command[1:4] == ["-m", "pip", "install"]:
            ...
            assert "skills-orchestrator==4.8.34 --hash=sha256:" in lock_text
