...
def test_build_allows_matching_provenance_content_hash(tmp_path):
    content = "---\nid: external-skill\n---\n# External Skill\n\nTrusted content.\n"
    config_path = _write_provenance_workspace(tmp_path, content=content)

    runner = CliRunner()
    output = tmp_path / "AGENTS.md"
    result = runner.invoke(cli, ["build", "--config", str(config_path), "--output", str(output)])

    assert result.exit_code == 0, result.output
    assert "Trusted content" in output.read_text(encoding="utf-8")


def test_build_rejects_mismatched_provenance_content_hash_without_leaking_content(tmp_path):
    content = (
        "---\nid: external-skill\n---\n# External Skill\n\nTAMPERED CONTENT SHOULD NOT LEAK.\n"
    )
    config_path = _write_provenance_workspace(tmp_path, content=content, expected_hash="a" * 64)

    runner = CliRunner()
    result = runner.invoke(cli, ["build", "--config", str(config_path)])
