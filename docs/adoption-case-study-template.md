# Adoption Case Study Template

Use this template after a real repository has produced a validated
`external-adoption-record`. It is designed to keep public adoption claims honest while still giving
...
List the smallest patch used for the adoption:
...
If the adoption required additional files, explain why:
...
Reference the validated `adoption-record.json` path or CI artifact URL if it is public.
...
Use neutral language. Do not claim productivity gains unless the adoption measured them.
...
> No quantitative outcome was measured during this adoption.
...
- This adoption validates CI governance of AI instruction artifacts.
...
- `adoption-record.json` validates against `external-adoption-record`.
