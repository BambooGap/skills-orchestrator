...
docker run --rm \
  -v "$PWD:/workspace" \
  -w /workspace \
  ghcr.io/bamboogap/skills-orchestrator:v4.8.39 \
...
3. Promote the verified digest into an internal registry or image cache:
   ...
   docker tag "${IMAGE}@sha256:<verified-digest>" internal.example.com/skillops/skills-orchestrator:v4.8.39
   docker push internal.example.com/skillops/skills-orchestrator:v4.8.39
   ```
