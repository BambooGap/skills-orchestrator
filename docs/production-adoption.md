...
| PyPI CLI | Pin an exact version such as `skills-orchestrator==4.8.35`. |
| Docker image | Pin the GHCR digest, not only `:v4.8.35`. |
...
```bash
git ls-remote https://github.com/BambooGap/skills-orchestrator.git refs/tags/v4.8.35
```

Resolve the image digest:

```bash
docker buildx imagetools inspect ghcr.io/bamboogap/skills-orchestrator:v4.8.35
```
...
```bash
python3.12 -m pip install "skills-orchestrator[mcp]==4.8.35"
```
