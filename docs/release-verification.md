...
Use the release tag as the `version` input, for example `v4.8.31`. The workflow runs `full_smoke`
...
`skills-orchestrator==4.8.31` is still only an exact version pin, but the full post-release smoke now
