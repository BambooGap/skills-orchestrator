...
[![GitHub Action](https://img.shields.io/badge/GitHub%20Action-v4.8.44-blue?logo=githubactions&logoColor=white)](docs/github-action.md)
...
|---------|----------------|-------------|
| OSS CLI | `v4.8.44` on PyPI | `python3.12 -m pip install skills-orchestrator` |
| GitHub Action | `v4.8.44` release tag | `BambooGap/skills-orchestrator@v4.8.44` |
| Container image | Published on GHCR | `ghcr.io/bamboogap/skills-orchestrator:v4.8.44` |
...
```bash
docker run --rm ghcr.io/bamboogap/skills-orchestrator:v4.8.44 --version
```
...
jobs:
  skills:
    ...
    steps:
      ...
      - uses: BambooGap/skills-orchestrator@v4.8.44
        with:
          config: config/skills.yaml
          policy-pack: builtin/team-standard
          upload-sarif: true
