...
[![GitHub Action](https://img.shields.io/badge/GitHub%20Action-v4.6.1-blue?logo=githubactions&logoColor=white)](docs/github-action.md)
...
|---------|----------------|-------------|
| OSS CLI | `v4.6.1` on PyPI | `python3.12 -m pip install skills-orchestrator` |
| GitHub Action | `v4.6.1` release tag | `BambooGap/skills-orchestrator@v4.6.1` |
| Container image | Published on GHCR | `ghcr.io/bamboogap/skills-orchestrator:v4.6.1` |
...
```bash
docker run --rm ghcr.io/bamboogap/skills-orchestrator:v4.6.1 --version
```
...
jobs:
  skills:
    ...
    steps:
      ...
      - uses: BambooGap/skills-orchestrator@v4.6.1
        with:
          config: config/skills.yaml
          policy-pack: builtin/team-standard
          upload-sarif: true
