...
[![GitHub Action](https://img.shields.io/badge/GitHub%20Action-v4.3.0-blue?logo=githubactions&logoColor=white)](docs/github-action.md)
...
|---------|----------------|-------------|
| OSS CLI | `v4.3.0` on PyPI | `python3.12 -m pip install skills-orchestrator` |
| GitHub Action | `v4.3.0` release tag | `BambooGap/skills-orchestrator@v4.3.0` |
| Container image | Published on GHCR | `ghcr.io/bamboogap/skills-orchestrator:v4.3.0` |
...
```bash
docker run --rm ghcr.io/bamboogap/skills-orchestrator:v4.3.0 --version
```
...
2. 先跑 `doctor --profile adopter` 和 `conformance run --profile core`。
...
- v4.x：补齐 CI explainability、schema audit、digest-bound container SBOM/provenance、GHCR attestation、Claude Skills round-trip export 和 release evidence polish。

### 下一阶段

- 增加更多真实生态 adapter examples，包括 Claude Skills bundle fixture、MCP client config 和 OpenAI Agents SDK scaffold 的跨仓 smoke。
- 继续推进 hash-locked dependency install、image signing 和更完整的 release verification docs。
