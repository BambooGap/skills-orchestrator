...
[![GitHub Action](https://img.shields.io/badge/GitHub%20Action-v4.7.3-blue?logo=githubactions&logoColor=white)](docs/github-action.md)
...
|---------|----------------|-------------|
| OSS CLI | `v4.7.3` on PyPI | `python3.12 -m pip install skills-orchestrator` |
| GitHub Action | `v4.7.3` release tag | `BambooGap/skills-orchestrator@v4.7.3` |
| Container image | Published on GHCR | `ghcr.io/bamboogap/skills-orchestrator:v4.7.3` |
...
```bash
docker run --rm ghcr.io/bamboogap/skills-orchestrator:v4.7.3 --version
```
...
jobs:
  skills:
    ...
    steps:
      ...
      - uses: BambooGap/skills-orchestrator@v4.7.3
        with:
          config: config/skills.yaml
          policy-pack: builtin/team-standard
          upload-sarif: true
...
- 继续推进 hash-locked dependency install、image signing、真实 GHCR attestation verification docs 和 OpenSSF Scorecard hygiene。
